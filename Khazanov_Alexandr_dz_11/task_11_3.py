...
class IsNumbers:
    ...
    def add_elem():
        ...
        list_inp = []
        while inp_val != 'q':
            inp_val = input("Число для списка (q - выход): ")
            try:
                inp_val = float(inp_val)
                list_inp.append(inp_val)
            except ValueError:
                if inp_val != 'q':
                    print("Вводите только числа. Этот элемент не будет включен")
                else:
                    return "Введенный список чисел: ", list_inp


print(IsNumbers().add_elem())
